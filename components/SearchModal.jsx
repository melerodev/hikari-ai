...
function getTimeGroup(dateString, t) {
  ...
  if (date >= sevenDaysAgo) return t("previous7Days")
  return t("older")
}
...
export default function SearchModal({
  ...
  const groupedConversations = useMemo(() => {
    ...
    groups[t("today")] = []
    groups[t("yesterday")] = []
    groups[t("previous7Days")] = []
    groups[t("older")] = []

    filteredConversations
      .sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt))
      .forEach((conv) => {
        const group = getTimeGroup(conv.updatedAt, t)
        if (groups[group]) {
        ...
        <>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 pointer-events-none">
            <motion.div
              ...
              className="w-full max-w-2xl rounded-2xl border border-zinc-200 bg-white shadow-2xl dark:border-zinc-800 dark:bg-zinc-900 pointer-events-auto"
            >
              {/* Search Header */}
              ...
              <div className="max-h-[60vh] overflow-y-auto">
                      ...
                      <div className="space-y-1">
                        {convs.map((conv) => (
                          <button
                            ...
                            className="flex w-full items-center gap-3 rounded-lg p-3 text-left hover:bg-zinc-100 dark:hover:bg-zinc-800"
                            aria-label={`${t("select")} ${conv.title}`}
                          >
                ...
                {filteredConversations.length === 0 && query.trim() && (
                  <div className="p-8 text-center">
                    ...
                    <div className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">{t("tryDifferentKeywords")}</div>
                  </div>
                )}
                ...
                {!query.trim() && conversations.length === 0 && (
                  <div className="p-8 text-center">
                    ...
                    <div className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">{t("startNewChatToBegin")}</div>
                  </div>
                )}
              </div>
            </motion.div>
